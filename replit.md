...
- June 30, 2025. Implemented comprehensive YouTube API quota handling system: added quota_exceeded status detection, enhanced scheduler to skip quota-exceeded content, implemented automatic retry protection, and ensured proper status persistence in database. System now gracefully handles YouTube API daily quota limits (10,000 units) and prevents duplicate publication attempts when quota is exceeded.
- June 30, 2025. Enhanced scheduler error handling system: implemented intelligent filtering to skip configuration errors (missing settings, missing images), old failed statuses (>24h), and quota exceeded statuses. System now prevents spam retry attempts for persistent errors while allowing retries for temporary issues. Added comprehensive error detection for "настройки не найдены", "отсутствует изображение", and similar configuration problems.
