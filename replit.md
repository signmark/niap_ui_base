...
- July 18, 2025. FIXED ERROR DISPLAY BUG IN UI: Resolved critical issue where scheduled posts with `error: null` showed "Ошибка" (Error) text in UI tooltips and status displays. Updated ScheduledPostInfo.tsx and related components to only show error messages when error field contains actual error information, not fallback text for null values. System now properly displays "Ожидает публикации" (Awaiting publication) for pending status instead of incorrect "Ошибка" text.
- July 18, 2025. REMOVED ERROR NULL FROM PLATFORM CREATION: Eliminated `error: null` field from being added during platform scheduling creation. Updated SocialContentAdaptationDialog.tsx and content/index.tsx to not include error field when creating pending status for platforms. System now creates clean platform objects without unnecessary null error fields that caused UI display issues.
