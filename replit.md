...
- July 3, 2025. COMMENT COLLECTION UI IMPLEMENTED: Added comment collection feature to trends interface with dual platform selection - "Платформы для сбора трендов/источников" for main trend collection and "Платформы для сбора комментариев трендов" for comment collection from existing trends. Updated SocialNetworkSelectorDialog with standardized variable naming (collectComments, selectedCommentPlatforms). Enhanced server-side /api/trends/collect endpoint to process collectComments parameter and pass it to N8N webhook for automated comment collection processing. System now supports collecting both trends and their associated comments through unified N8N workflow architecture.
