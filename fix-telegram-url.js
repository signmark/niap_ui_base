 ...
 * Обрабатывает URL вида https://t.me/c/2302366310 -> https://t.me/ya_delayu_moschno
 * 
 * Запуск: node fix-telegram-url.js
 */
...
const TELEGRAM_CHANNEL_USERNAME = "ya_delayu_moschno"; // Укажите username вашего канала
...
function log(message) {
  ...
  fs.appendFileSync('fix-telegram-url.log', `[${timestamp}] ${message}\n`);
}
...
/**
 * Фиксит неполный URL Telegram, заменяя его на URL с username канала
 * @param {object} content Объект контента
...
function fixTelegramUrl(content) {
  try {
    ...
    // Заменяем URL на URL с username канала
    updatedPlatforms.telegram = {
      ...updatedPlatforms.telegram,
      postUrl: `https://t.me/${TELEGRAM_CHANNEL_USERNAME}`
    };
...
async function main() {
  // Создаем файл логов
  if (!fs.existsSync('fix-telegram-url.log')) {
    fs.writeFileSync('fix-telegram-url.log', '');
  }
