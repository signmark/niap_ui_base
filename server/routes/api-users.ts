...
usersRouter.get('/active', isSmmAdmin, async (req: Request, res: Response) => {
  try {
    ...
    console.log(`[api-users] UserId: ${(req as any).userId}, isSmmAdmin: ${(req as any).isSmmAdmin}`);
    
    // Используем токен из запроса
    const token = req.headers.authorization?.replace('Bearer ', '');
    
    if (!token) {
      console.log('[api-users] Отсутствует токен авторизации');
    ...
    // Логируем часть токена (для безопасности только первые 10 символов)
    console.log(`[api-users] Используем токен: ${token.substring(0, 10)}...`);
