...
router.get('/admin/users', async (req: any, res: Response) => {
  try {
    ...
    // Для получения списка пользователей используем административный токен из переменных окружения
    // так как обычные пользователи не имеют прав на чтение всех пользователей в Directus
    ...
    console.log('[admin-users] Используем системный административный токен для получения пользователей');
...
router.get('/admin/users/activity', async (req, res) => {
  try {
    ...
    // Для получения статистики используем административный токен из переменных окружения
    const adminToken = process.env.DIRECTUS_TOKEN;
    if (!adminToken) {
    ...
    console.log('[admin-users] Используем системный административный токен для получения статистики');
