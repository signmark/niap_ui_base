...
export function registerClaudeRoutes(app: Router) {
   ...
   * Используется новый обработчик в routes.ts с данными кампании
   */
  router.post('/api/claude/generate-content', async (req: Request, res: Response) => {
    console.log(`🚨🚨🚨 БЛОКИРОВАНО! Старый обработчик Claude больше не используется! 🚨🚨🚨`);
    console.log(`🎯 Используйте новый обработчик /api/generate-content с данными кампании`);
    return res.status(410).json({
      success: false,
      error: 'Этот обработчик отключен. Используйте /api/generate-content с данными кампании',
      redirectTo: '/api/generate-content'
    });
    
    // ВЕСЬ КОД НИЖЕ ЗАБЛОКИРОВАН
    /*
    try {
