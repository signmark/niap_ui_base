...
export class SocialPublishingService {
  ...
  async publishToInstagram(
    ...
    try {
      ...
      // Instagram не поддерживает HTML-теги, преобразуем их в текст и эмодзи
      const contentText = processedContent.content
        ...
        // Обрабатываем форматирование текста с эмодзи
        .replace(/<b>(.*?)<\/b>/g, '𝗕𝗼𝗹𝗱: $1')
        .replace(/<strong>(.*?)<\/strong>/g, '𝗕𝗼𝗹𝗱: $1')
        .replace(/<i>(.*?)<\/i>/g, '𝘐𝘵𝘢𝘭𝘪𝘤: $1')
        .replace(/<em>(.*?)<\/em>/g, '𝘐𝘵𝘢𝘭𝘪𝘤: $1')
