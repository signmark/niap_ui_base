...
export class SocialPublishingService {
  ...
  async publishToInstagram(
    ...
    try {
      ...
      // Instagram не поддерживает HTML-теги, поэтому просто преобразуем весь HTML в обычный текст
      // без добавления каких-либо символов форматирования
      const contentText = processedContent.content
        // Обрабатываем блочные элементы для правильных переносов
        .replace(/<br\s*\/?>/g, '\n')
        .replace(/<p>(.*?)<\/p>/g, '$1\n\n')
        .replace(/<div>(.*?)<\/div>/g, '$1\n')
        .replace(/<h[1-6]>(.*?)<\/h[1-6]>/g, '$1\n\n') // Заголовки без специальных символов
        ...
        // Убираем все теги форматирования без замены на специальные символы
        .replace(/<b>(.*?)<\/b>/g, '$1')
        .replace(/<strong>(.*?)<\/strong>/g, '$1')
        .replace(/<i>(.*?)<\/i>/g, '$1')
        .replace(/<em>(.*?)<\/em>/g, '$1')
