...
export class SocialPublishingService {
  ...
  async publishToVk(
    ...
    try {
      ...
      // VK не поддерживает HTML-теги, но предлагает свою разметку текста
      // Преобразуем HTML в нативную разметку ВКонтакте:
      // *жирный*, _курсив_, [ссылка|текст]
      let contentText = processedContent.content
        // Обрабатываем блочные элементы для правильных переносов
        .replace(/<br\s*\/?>/g, '\n')
        .replace(/<p>(.*?)<\/p>/g, '$1\n\n')
      ...
      // Преобразуем HTML теги в текстовые маркеры ВКонтакте
      contentText = contentText
        // Жирный текст: <b> и <strong> -> *текст*
        .replace(/<b>(.*?)<\/b>/g, '*$1*')
        .replace(/<strong>(.*?)<\/strong>/g, '*$1*')
        
        // Курсив: <i> и <em> -> _текст_
        .replace(/<i>(.*?)<\/i>/g, '_$1_')
        .replace(/<em>(.*?)<\/em>/g, '_$1_')
        
        // Ссылки: <a href="url">текст</a> -> [url|текст]
        .replace(/<a\s+href="(.*?)".*?>(.*?)<\/a>/g, '[$1|$2]')
        
        // Списки: обрабатываем каждый элемент списка
        .replace(/<li>(.*?)<\/li>/g, '• $1\n')
        
        // Подчеркнутый и зачеркнутый - убираем теги, но сохраняем содержимое
        .replace(/<u>(.*?)<\/u>/g, '$1')
        .replace(/<s>(.*?)<\/s>/g, '$1')
        .replace(/<strike>(.*?)<\/strike>/g, '$1')
        
        // Удаляем все прочие HTML-теги, но сохраняем их содержимое
        .replace(/<\/?[^>]+(>|$)/g, '');
      
      log(`Обработанный текст для VK (конвертированный из HTML): ${contentText.substring(0, 100)}...`, 'social-publishing');
  ...
  async publishToInstagram(
    ...
    try {
      ...
      // Instagram не поддерживает HTML-теги явно, но можно использовать Unicode символы и эмодзи для визуального форматирования
      const contentText = processedContent.content
        // Обрабатываем блочные элементы для правильных переносов
        .replace(/<br\s*\/?>/g, '\n')
        .replace(/<p>(.*?)<\/p>/g, '$1\n\n')
        .replace(/<div>(.*?)<\/div>/g, '$1\n')
        .replace(/<h[1-6]>(.*?)<\/h[1-6]>/g, '⭐ $1 ⭐\n\n') // Используем звездочку вместо ✨
        ...
        // Обрабатываем форматирование текста с использованием Unicode-символов
        // Более аккуратный подход с сохранением важного форматирования
        .replace(/<b>([^<]{1,30})<\/b>/g, '𝐁 $1')  // Жирный стиль для коротких фраз 
        .replace(/<strong>([^<]{1,30})<\/strong>/g, '𝐁 $1')
        .replace(/<i>([^<]{1,30})<\/i>/g, '𝐼 $1')  // Курсив для коротких фраз
        .replace(/<em>([^<]{1,30})<\/em>/g, '𝐼 $1')
        
        // Для длинных фрагментов (обычно целых параграфов) просто сохраняем текст
        .replace(/<b>([^<]{31,})<\/b>/g, '$1')
