...
export class YouTubeService extends BaseSocialService {
  ...
  async publishContent(
    ...
    try {
      ...
      // Обработка ошибок квот - проверяем разные варианты сообщений
      if (error.message?.includes('quotaExceeded') || 
          error.message?.includes('you have exceeded your quota') ||
          error.message?.includes('quota')) {
        return {
          success: false,
          error: 'Превышена квота YouTube API. Попробуйте позже.',
          quotaExceeded: true // специальный флаг для планировщика
        };
      }
