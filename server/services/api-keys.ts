  ...
  async fixDeepSeekKeyFormat(userId: string, authToken?: string): Promise<boolean> {
    try {
      ...
      if (deepseekKeyItem && deepseekKeyItem.api_key) {
        ...
        // Проверяем формат ключа - DeepSeek ключи хранятся БЕЗ префикса "Bearer "
        ...
        console.log(`[deepseek] Ключ не найден в БД для пользователя ${userId}`);
        return false;
      }
    } catch (error) {
      console.error(`[deepseek] Ошибка при обновлении формата ключа:`, error);
      return false;
    }
  }
}
