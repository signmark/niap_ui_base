...
Return only the translated text, no explanations or comments.`;
  ...
  async generateImage(
    ...
    try {
      ...
      // НЕ меняем формат ключа, используем как есть (предполагается, что ключ уже в формате "Key xxxxxx:xxxxx")
      // FAL.AI API требует формат "Key {apiKey}", НЕ используем Bearer
      const authHeader = this.apiKey;
      console.log(`[FAL.AI] Заголовок авторизации: используем ключ как есть, без изменений`);
          ...
          while ((Date.now() - startTime) / 1000 < maxWaitTime) {
            try {
              ...
              statusResponse = await axios.get(response.data.status_url, {
                headers: {
                  'Authorization': statusAuthHeader, // Используем API ключ как есть, предполагая формат "Key {apiKey}"
                  'Accept': 'application/json'
                }
              });
              ...
              if (statusResponse.data?.status === "COMPLETED") {
                ...
                const resultResponse = await axios.get(response.data.response_url, {
                  headers: {
                    'Authorization': resultAuthHeader, // Используем API ключ как есть
                    'Accept': 'application/json'
                  }
                });
  ...
  async initialize(userId: string, authToken?: string): Promise<boolean> {
    try {
      ...
      if (apiKey) {
        // Подробное логирование для диагностики API ключа
        console.log(`🔍 ПОЛУЧЕН API КЛЮЧ FAL.AI: ${apiKey}`);
        ...
        // Сохраняем ключ БЕЗ ИЗМЕНЕНИЙ в исходном формате
        this.updateApiKey(apiKey);
        
        log('FAL.AI API ключ успешно получен через API Key Service', 'fal-ai');
        console.log('FAL.AI API ключ успешно получен через API Key Service');
        return true;
      } else {
        ...
        log('FAL.AI API ключ не найден', 'fal-ai');
