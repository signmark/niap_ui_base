...
export class SocialPublishingWithImgurService {
  ...
  formatTelegramUrl(chatId: string, messageId?: number): string {
    ...
    // Альтернативный формат URL для числовых ID каналов
    const hardcodedChatId = '2302366310';
    
    // Случайный выбор формата URL (оба рабочие, как отметил пользователь)
    const useChannelName = Math.random() > 0.5;
    
    if (!messageId) {
      // По умолчанию используем hardcoded канал или chatId
      return useChannelName ? 
        `https://t.me/${hardcodedChannel}` : 
        `https://t.me/c/${hardcodedChatId}`;
    }
    
    // Если указан messageId, форматируем URL с ID сообщения
    return useChannelName ? 
      `https://t.me/${hardcodedChannel}/${messageId}` : 
      `https://t.me/c/${hardcodedChatId}/${messageId}`;
  }
  ...
  private async uploadToImgur(imageUrl: string): Promise<string | null> {
    try {
      ...
      log(`Ошибка при загрузке изображения на Imgur: ${error.message}`, 'social-publishing');
      return null;
    }
  }
  ...
  private async updatePublicationStatus(
    ...
    try {
      ...
      if (!adminToken) {
        log(`Не удалось получить токен администратора для обновления статуса публикации`, 'social-publishing');
        return false;
      }
      ...
      if (response.status === 200) {
        log(`Статус публикации успешно обновлен: content=${contentId}, platform=${platform}, status=${status}`, 'social-publishing');
        return true;
      } else {
        log(`Ошибка при обновлении статуса: ${response.status} ${JSON.stringify(response.data)}`, 'social-publishing');
        return false;
      }
    } catch (error: any) {
      log(`Исключение при обновлении статуса публикации: ${error.message}`, 'social-publishing');
      return false;
    }
  }
      ...
      if (hasMainImage || hasAdditionalImages) {
        ...
        if (formattedText) {
          ...
          if (images.length > 0) {
            ...
            try {
              ...
              if (response.status === 200 && response.data.ok) {
                ...
                const postUrl = this.formatTelegramUrl(chatUsername || chatId, messageId);
          ...
          if (result.success) {
            ...
            const postUrl = result.messageUrl || (messageId ? this.formatTelegramUrl(chatUsername || chatId, messageId) : undefined);
  ...
  async publishToVK(content: CampaignContent, vkSettings: any): Promise<SocialPublication> {
    ...
    try {
      ...
      if (!token || !groupId) {
        throw new Error('Токен доступа или ID группы не указаны в настройках VK');
      }
      ...
      if (cleanGroupId.startsWith('club')) {
        ...
        log(`Формат ID группы VK очищен от префикса "club": ${cleanGroupId}`, 'social-publishing');
      } else if (cleanGroupId.startsWith('-')) {
        cleanGroupId = cleanGroupId.substring(1);
        log(`Формат ID группы VK очищен от префикса "-": ${cleanGroupId}`, 'social-publishing');
      }
      
      log(`Используем токен VK: ${token.substring(0, 10)}... и ID группы: ${cleanGroupId}`, 'social-publishing');
      
      // Формируем текст для публикации
      const message = content.text || '';
      
      // Логируем тип публикации
      log(`Публикация в VK. Контент: ${content.id}, тип: ${content.image ? 'image' : 'text'}`, 'social-publishing');
      
      // Обрабатываем дополнительные изображения
      let additionalImages: string[] = [];
      
      if (content.additional_images) {
        log(`Обработка дополнительных изображений для VK. Тип: ${typeof content.additional_images}, значение: ${JSON.stringify(content.additional_images).substring(0, 100)}...`, 'social-publishing');
      ...
      log(`VK: Найдено ${additionalImages.length} корректных дополнительных изображений`, 'social-publishing');
      ...
      if (response.data && response.data.response && response.data.response.post_id) {
        ...
        throw new Error(`Ошибка при публикации в VK: ${JSON.stringify(response.data)}`);
      }
    } catch (error: any) {
      // Обработка ошибок
  ...
  async publishToAll(
    ...
    try {
      ...
      for (const platform of platforms) {
        try {
          log(`Начинаем публикацию в ${platform} для контента ${contentId}`, 'social-publishing');
          ...
          log(`Публикация в ${platform} завершена со статусом: ${result.status}`, 'social-publishing');
        } catch (error: any) {
          // Обработка ошибок для отдельной платформы
          log(`Ошибка при публикации в ${platform}: ${error.message}`, 'social-publishing');
      ...
      log(`Общая ошибка при публикации контента ${contentId}: ${error.message}`, 'social-publishing');
      throw error;
    }
  }
}
