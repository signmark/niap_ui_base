...
import { instagramTestRouter } from './test-instagram-reels';
...
https://t.me/channelname/ - description`;
  ...
  // Регистрируем тестовый маршрут для Instagram Reels
  app.use('/api/instagram-reels-test', instagramTestRouter);
  console.log('[express] Instagram Reels тестовый маршрут успешно зарегистрирован');
  ...
  app.post("/api/generate-image", authenticateUser, async (req: any, res) => {
    try {
      ...
      const { 
        ...
        height = 1024, 
        campaignId,
        contentId, // ID контента для привязки изображений
        businessData,
        content,
        platform,
        numImages = 1,
        savePrompt = false // Флаг для сохранения промта в БД
      } = req.body;
      ...
      try {
        ...
        // Сначала сохраняем промт, если есть contentId и флаг savePrompt = true
        // Важно делать это ДО генерации, чтобы сохранить промт даже при ошибке генерации
