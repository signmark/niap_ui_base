  ...
  app.post("/api/generate-image", authenticateUser, async (req: any, res) => {
    try {
      ...
      try {
        ...
        // Сохраняем промт, если есть contentId и флаг savePrompt = true
        if (contentId && savePrompt && prompt) {
          console.log(`Сохраняем промт для контента с ID: ${contentId}`);
          try {
            ...
            // Продолжаем выполнение и не прерываем ответ с изображениями
          }
        }
