...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 🎯 НОВЫЙ РОУТ АНАЛИТИКИ С ПРАВИЛЬНЫМ ПОДСЧЕТОМ ПОСТОВ
  app.get('/api/analytics', authenticateUser, async (req: Request, res: Response) => {
    console.log(`🎯 [NEW-ANALYTICS] Новый роут аналитики запущен!`);
    
    const { campaignId, period = '7days' } = req.query;
    console.log(`🎯 [NEW-ANALYTICS] Параметры: campaignId=${campaignId}, period=${period}`);
    ...
    try {
      // Запрос к Directus для получения постов за указанный период
      const periodDays = period === '30days' ? 30 : 7;
      const url = `https://directus.nplanner.ru/items/campaign_content?filter[campaign_id][_eq]=${campaignId}&filter[status][_eq]=published&filter[published_at][_gte]=$NOW(-${periodDays} days)`;
      
      console.log(`🎯 [NEW-ANALYTICS] Запрос к Directus: ${url}`);
      ...
      // ПРАВИЛЬНАЯ ЛОГИКА: считаем количество ПОСТОВ, а не публикаций на платформах
      const totalPosts = posts.length;
      
      console.log(`🎯 [NEW-ANALYTICS] Найдено ${totalPosts} постов за ${periodDays} дней`);

      // Собираем статистику по платформам
      let totalViews = 0;
      let totalLikes = 0;
      let totalShares = 0;
      let totalComments = 0;
      ...
      const platformStats = {
        ...
        Instagram: { posts: 0, views: 0, likes: 0, shares: 0, comments: 0 }
      };

      // Обрабатываем каждый пост
      posts.forEach((post: any) => {
        const platforms = new Set(); // Множество для отслеживания уникальных платформ для этого поста
        ...
        if (post.social_platforms) {
          Object.values(post.social_platforms).forEach((platform: any) => {
            if (platform.status === 'published') {
              ...
              if (platformName && platformStats[platformName] && !platforms.has(platformName)) {
                platforms.add(platformName);
                platformStats[platformName].posts++; // Увеличиваем счетчик постов для платформы

                if (platform.analytics) {
                  const analytics = platform.analytics;
                  platformStats[platformName].views += analytics.views || 0;
                  platformStats[platformName].likes += analytics.likes || 0;
                  platformStats[platformName].shares += analytics.shares || 0;
                  platformStats[platformName].comments += analytics.comments || 0;
      ...
      const result = {
        ...
        data: {
          ...
          totalShares,
          totalComments,
          totalPosts // ПРАВИЛЬНОЕ количество постов!
        }
      };

      console.log(`🎯 [NEW-ANALYTICS] РЕЗУЛЬТАТ: totalPosts=${totalPosts} (ПРАВИЛЬНО!)`);
      
      res.json(result);

    } catch (error: any) {
      console.error('🎯 [NEW-ANALYTICS] Ошибка:', error.message);
