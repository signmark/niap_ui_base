...
async function extractFullSiteContent(url: string): Promise<string> {
  const startTime = Date.now();
  try {
    console.log(`🚀 Расширенный анализ сайта: ${url}`);
    ...
    try {
      // Первая попытка с большими лимитами
    ...
    // Заголовки с HTML-тегами
    const h1s = htmlContent.match(/<h1[^>]*>([^<]+)<\/h1>/gi)?.map(h => h.replace(/<[^>]+>/g, '').trim()).filter(Boolean).slice(0, 8) || [];
    const h2s = htmlContent.match(/<h2[^>]*>([^<]+)<\/h2>/gi)?.map(h => h.replace(/<[^>]+>/g, '').trim()).filter(Boolean).slice(0, 10) || [];
    const h3s = htmlContent.match(/<h3[^>]*>([^<]+)<\/h3>/gi)?.map(h => h.replace(/<[^>]+>/g, '').trim()).filter(Boolean).slice(0, 12) || [];
    
    // Параграфы с содержательным текстом (упрощенный regex)
    const paragraphs = htmlContent.match(/<p[^>]*>([^<]+)<\/p>/gi)
      ?.map(p => p.replace(/<[^>]+>/g, '').trim())
      .filter(text => text.length > 20 && text.length < 500 && !text.includes('cookie') && !text.includes('JavaScript'))
      .slice(0, 15) || [];
    
    // Списки (важные для описания услуг) 
    const listItems = htmlContent.match(/<li[^>]*>([^<]+)<\/li>/gi)
      ?.map(li => li.replace(/<[^>]+>/g, '').trim())
      .filter(text => text.length > 10 && text.length < 200)
      .slice(0, 10) || [];
    
    // Специфичные для SMM/бизнеса секции
    const businessSections = [];
    
    // Поиск секций с преимуществами
    const advantagePatterns = [
      /(?:преимущества|advantages|benefits|особенности)[^<]*<[^>]*>([^<]+)/gi,
      /(?:возможности|capabilities|features)[^<]*<[^>]*>([^<]+)/gi,
      /(?:решения|solutions|services)[^<]*<[^>]*>([^<]+)/gi
    ];
    ...
    advantagePatterns.forEach(pattern => {
      ...
      if (matches) {
        matches.slice(0, 5).forEach(match => {
          ...
          if (text.length > 10) businessSections.push(text);
        });
      }
    });
    
    // Определение типа бизнеса по контенту
    let businessType = 'общая';
    const contentLower = (title + ' ' + description + ' ' + h1s.join(' ')).toLowerCase();
    
    if (contentLower.includes('smm') || contentLower.includes('социальн') || contentLower.includes('реклам') || 
        contentLower.includes('маркетинг') || contentLower.includes('продвижен')) {
      businessType = 'SMM/Маркетинг';
    } else if (contentLower.includes('разработ') || contentLower.includes('программ') || contentLower.includes('сайт')) {
      businessType = 'IT/Разработка';
    } else if (contentLower.includes('еда') || contentLower.includes('ресторан') || contentLower.includes('кафе')) {
      businessType = 'Ресторанный бизнес';
    } else if (contentLower.includes('здоров') || contentLower.includes('медиц') || contentLower.includes('диет')) {
      businessType = 'Здравоохранение';
    }
    
    // ФОРМИРУЕМ БОГАТЫЙ РЕЗУЛЬТАТ
    const result = [
      `URL: ${url}`,
      ...
      h1s.length > 0 ? `ОСНОВНЫЕ ЗАГОЛОВКИ (H1): ${h1s.join(' | ')}` : '',
      h2s.length > 0 ? `РАЗДЕЛЫ (H2): ${h2s.join(' | ')}` : '',
      h3s.length > 0 ? `ПОДРАЗДЕЛЫ (H3): ${h3s.join(' | ')}` : '',
      paragraphs.length > 0 ? `ОСНОВНОЙ КОНТЕНТ:\n${paragraphs.join('\n\n')}` : '',
      listItems.length > 0 ? `СПИСКИ УСЛУГ/ОСОБЕННОСТЕЙ:\n- ${listItems.join('\n- ')}` : '',
      businessSections.length > 0 ? `БИЗНЕС-ИНФОРМАЦИЯ:\n${businessSections.join('\n')}` : ''
    ].filter(Boolean).join('\n\n');
    
    console.log(`✅ Расширенный анализ завершен за ${Date.now() - startTime}ms (${result.length} символов)`);
    
    // Ограничиваем размер, но оставляем больше данных для AI
    return result.substring(0, 15000); // Увеличил до 15KB для лучшего анализа
    
  } catch (error: any) {
    console.error(`❌ Ошибка анализа сайта ${url}:`, error.message);
    
    // Умный fallback на основе URL для любых типов сайтов
    const domain = url.replace(/^https?:\/\//, '').split('/')[0].toLowerCase();
    let smartFallback = `URL: ${url}\n\nКОНТЕНТ НЕДОСТУПЕН - ИНТЕЛЛЕКТУАЛЬНАЯ ОБРАБОТКА:\n\n`;
    
    // Расширенная логика определения типа бизнеса по домену
    if (domain.includes('smm') || domain.includes('social') || domain.includes('marketing')) {
      smartFallback += `ТИП БИЗНЕСА: SMM/Маркетинг
НАЗВАНИЕ: ${domain.split('.')[0].toUpperCase()}
ОПИСАНИЕ: Платформа для управления социальными сетями и маркетинга
УСЛУГИ: SMM-услуги, продвижение в социальных сетях, создание контента
ЦЕЛЕВАЯ АУДИТОРИЯ: SMM-специалисты, маркетологи, бизнес
ЦЕННОСТИ: Эффективное продвижение, качественный контент, результативность
ФИЛОСОФИЯ: Социальные сети - ключ к успешному бизнесу`;
    } else if (domain.includes('shop') || domain.includes('store') || domain.includes('market')) {
      smartFallback += `ТИП БИЗНЕСА: Интернет-магазин
НАЗВАНИЕ: ${domain.split('.')[0].charAt(0).toUpperCase() + domain.split('.')[0].slice(1)}
ОПИСАНИЕ: Онлайн-продажи товаров и услуг
УСЛУГИ: Продажа товаров, доставка, консультации
ЦЕЛЕВАЯ АУДИТОРИЯ: Покупатели, клиенты
ЦЕННОСТИ: Качество товаров, сервис, удобство покупок
ФИЛОСОФИЯ: Клиент всегда прав, качество превыше всего`;
    } else if (domain.includes('med') || domain.includes('health') || domain.includes('clinic')) {
      smartFallback += `ТИП БИЗНЕСА: Медицина/Здравоохранение
НАЗВАНИЕ: ${domain.split('.')[0].charAt(0).toUpperCase() + domain.split('.')[0].slice(1)}
ОПИСАНИЕ: Медицинские услуги и консультации
УСЛУГИ: Медицинская помощь, диагностика, лечение
ЦЕЛЕВАЯ АУДИТОРИЯ: Пациенты, медицинские работники
ЦЕННОСТИ: Здоровье пациентов, профессионализм, забота
ФИЛОСОФИЯ: Здоровье - главная ценность человека`;
    } else if (domain.includes('food') || domain.includes('restaurant') || domain.includes('cafe')) {
      smartFallback += `ТИП БИЗНЕСА: Ресторанный бизнес
НАЗВАНИЕ: ${domain.split('.')[0].charAt(0).toUpperCase() + domain.split('.')[0].slice(1)}
ОПИСАНИЕ: Ресторанные услуги и общественное питание
УСЛУГИ: Приготовление пищи, обслуживание, доставка
ЦЕЛЕВАЯ АУДИТОРИЯ: Посетители ресторанов, любители хорошей кухни
ЦЕННОСТИ: Качественные продукты, вкус, гостеприимство
ФИЛОСОФИЯ: Еда объединяет людей, качество превыше всего`;
    } else if (domain.includes('edu') || domain.includes('school') || domain.includes('university')) {
      smartFallback += `ТИП БИЗНЕСА: Образование
НАЗВАНИЕ: ${domain.split('.')[0].charAt(0).toUpperCase() + domain.split('.')[0].slice(1)}
ОПИСАНИЕ: Образовательные услуги и обучение
УСЛУГИ: Обучение, курсы, консультации, сертификация
ЦЕЛЕВАЯ АУДИТОРИЯ: Студенты, учащиеся, специалисты
ЦЕННОСТИ: Знания, развитие, профессионализм
ФИЛОСОФИЯ: Образование - путь к успеху`;
    } else if (domain.includes('tech') || domain.includes('it') || domain.includes('dev')) {
      smartFallback += `ТИП БИЗНЕСА: IT/Технологии
НАЗВАНИЕ: ${domain.split('.')[0].charAt(0).toUpperCase() + domain.split('.')[0].slice(1)}
ОПИСАНИЕ: IT-услуги и технологические решения
УСЛУГИ: Разработка ПО, консультации, техподдержка
ЦЕЛЕВАЯ АУДИТОРИЯ: Бизнес, IT-специалисты, частные клиенты
ЦЕННОСТИ: Инновации, качество кода, технологичность
ФИЛОСОФИЯ: Технологии улучшают жизнь людей`;
    } else {
      // Универсальный fallback для любых других доменов
      const businessName = domain.split('.')[0].charAt(0).toUpperCase() + domain.split('.')[0].slice(1);
      smartFallback += `ТИП БИЗНЕСА: Определяется по специфике домена
НАЗВАНИЕ: ${businessName}
ОПИСАНИЕ: Коммерческая или информационная деятельность
УСЛУГИ: Основные услуги определяются спецификой бизнеса
ЦЕЛЕВАЯ АУДИТОРИЯ: Целевая группа соответствует профилю деятельности
ЦЕННОСТИ: Качество, профессионализм, клиентоориентированность
ФИЛОСОФИЯ: Стремление к excellence в своей области`;
    }
...
${websiteContent}`;
          ...
          // ПРИОРИТЕТ 2: Специальная обработка для nplanner.ru - сервис диагностики здоровья и питания
            ...
            targetAudience = 'Диетологи, нутрициологи, врачи, специалисты по питанию, медицинские центры';
            businessValues = 'Персонализированная медицина, научный подход к питанию, профилактика заболеваний, качество диагностики';
            productBeliefs = 'Персонализированное питание на основе научных данных - ключ к здоровью и профилактике заболеваний';
            console.log(`[WEBSITE-ANALYSIS] 🔧 DEBUG: Определен тип: МЕДИЦИНСКИЕ ТЕХНОЛОГИИ (NPLANNER)`);
          } else if (contentLower.includes('сало') || contentLower.includes('свинина') || contentLower.includes('мясн') || contentLower.includes('деликатес')) {
