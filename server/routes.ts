...
`;
  ...
  // API для генерации изображений с помощью FAL.AI
  app.post("/api/generate-image", authenticateUser, async (req: any, res) => {
    try {
      const { prompt, negativePrompt, width, height, campaignId, businessData, content, platform, numImages } = req.body;
      const authHeader = req.headers['authorization'];
      ...
      console.log(`Запрос на генерацию изображения для кампании ${campaignId} от пользователя ${userId}`);
      
      // Инициализируем сервис FAL.AI с API ключом пользователя
      const isInitialized = await falAiService.initialize(userId, token);
      
      if (!isInitialized) {
        return res.status(400).json({
          success: false,
          error: "FAL.AI API ключ не настроен в профиле пользователя. Пожалуйста, добавьте API ключ в настройках."
        });
      }
      ...
      if (businessData) {
        // Генерация изображения на основе данных бизнеса
        console.log('Генерация изображений для бизнеса на основе анкеты');
        images = await falAiService.generateBusinessImage(businessData);
      } else if (content && platform) {
        // Генерация изображения для социальных сетей
        console.log(`Генерация изображений для контента в ${platform}`);
        images = await falAiService.generateSocialMediaImage(content, platform);
      } else if (prompt) {
        // Генерация по прямому запросу
        console.log(`Генерация изображений по запросу: "${prompt}"`);
        images = await falAiService.generateImage(prompt, {
          negativePrompt,
          width: width || 1024,
          height: height || 1024,
          numImages: numImages || 1
        ...
        return res.status(400).json({
          success: false,
          error: "Недостаточно данных для генерации изображения. Требуется prompt, businessData или content+platform."
        });
      }
