...
export function registerXmlRiverRoutes(app: Express): void {
  ...
  app.post('/api/xmlriver/save-key', authenticateXmlRiverRequest, async (req: Request, res: Response) => {
    try {
      ...
      // Вызываем сервис сохранения API ключа
      const saveResult = await apiKeyService.saveApiKey(
        userId,
        'xmlriver',
        formattedApiKey,
        token
      ...
      if (saveResult.success) {
        ...
        return res.status(500).json({
          ...
          message: saveResult.error || 'Неизвестная ошибка при сохранении ключа'
        });
      }
    } catch (error) {
      console.error(`[XMLRiver] Ошибка при сохранении API ключа: ${error instanceof Error ? error.message : 'Unknown error'}`);
      
      return res.status(500).json({
        success: false,
        error: 'Внутренняя ошибка сервера',
        message: 'Произошла ошибка при сохранении API ключа'
      });
    }
  });
  ...
  app.get('/api/xmlriver/keywords/:query', async (req: Request, res: Response) => {
    try {
      ...
      if (!query) {
        return res.status(400).json({
          success: false,
          error: 'Необходимо указать запрос',
          message: 'Параметр query обязателен'
        });
      }
      
      const result = await xmlRiverClient.searchKeywords(query);
      
      return res.status(200).json(result);
    } catch (error) {
      console.error(`[XMLRiver] Ошибка при поиске ключевых слов: ${error instanceof Error ? error.message : 'Unknown error'}`);
