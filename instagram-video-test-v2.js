/**
 * Тест публикации видео в Instagram через Graph API (версия 2)
 * 
 ...
 * 
 * Запуск: node instagram-video-test-v2.js [container_id]
 */
...
// Функция для логирования с меткой времени и записью в файл
function log(message) {
  const timestamp = new Date().toISOString().slice(11, 19);
  const logMessage = `${timestamp} [Instagram Test] ${message}`;
  console.log(logMessage);
  
  // Также записываем в файл
  fs.appendFileSync('instagram-video-publish.log', logMessage + '\n');
}
...
async function publishVideoToInstagram() {
  try {
    // Проверяем, передан ли ID контейнера в аргументах командной строки
    const passedContainerId = process.argv[2];
    
    if (passedContainerId) {
      log(`Получен ID контейнера из аргументов: ${passedContainerId}`);
      ...
      return await checkAndPublishVideo(passedContainerId, token, businessAccountId);
    }
    ...
    // URL видео для публикации (используется тестовое)
    const videoUrl = 'https://6e679636ae90-ridiculous-seth.s3.ru1.storage.beget.cloud/videos/converted_079fea9f-8d1e-4498-8708-827c657c9c4f.mp4';
...
fs.writeFileSync('instagram-video-publish.log', '=== Начало тестирования ===\n', { flag: 'w' });
