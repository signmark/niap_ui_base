...
export function ImageGenerationDialog({
  ...
  useEffect(() => {
    ...
    // Либо берем контент из текста статьи, либо оставляем пустое поле
    
    // По требованию пользователя не используем промт из предыдущего редактирования
    // Вместо этого используем введенный текст статьи как основу для промта
    if (initialContent) {
      // Очищаем теги из контента для создания базового промта
      const cleanPromptBase = simpleCleanHtml(initialContent);
      // Оставляем промт пустым, но подготавливаем контент для генерации
      setPrompt("");
      setGeneratedPrompt("");
      console.log('Подготовлен контент для генерации нового промта из текста статьи');
    } else {
      // Если контента нет, начинаем с пустого промта
      setPrompt("");
      setGeneratedPrompt("");
    ...
    // Всегда выбираем вкладку для генерации на основе контента, если контент есть
    if (initialContent) {
      // Всегда выбираем вкладку для генерации на основе контента
      setActiveTab("social"); // Переключаемся на вкладку социальных сетей
      console.log('Выбрана вкладка для генерации на основе текста');
    } else {
      // Только если нет контента, используем вкладку с прямым промтом
