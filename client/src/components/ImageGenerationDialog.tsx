...
export function ImageGenerationDialog({
  ...
  useEffect(() => {
    ...
    // ВАЖНО: Всегда оставляем поле промта пустым при открытии диалога
    // Это исключает случай, когда текст контента автоматически используется как промт
    // Пользователь сам введет нужный промт или сгенерирует его
    let promptToUse = "";
    let promptSource = "none";
    
    console.log('⚠️ Поле промпта оставлено пустым для ввода пользователем');
    
    // Для отладки логируем, какие значения были переданы, но не используем их
    if (promptText) {
      console.log('Найден promptText, но не используется:', promptText.substring(0, 30) + '...');
    }
    if (initialPrompt) {
      console.log('Найден initialPrompt, но не используется:', initialPrompt.substring(0, 30) + '...');
    }
    if (contentObject && contentObject.prompt) {
      console.log('Найден contentObject.prompt, но не используется:', contentObject.prompt.substring(0, 30) + '...');
    }
    
    // Оставляем поле промта пустым - пользователь сам введет или сгенерирует промт
    setPrompt(""); 
    setGeneratedPrompt("");
    console.log(`📝 Поле промта оставлено пустым для ввода пользователем`);
