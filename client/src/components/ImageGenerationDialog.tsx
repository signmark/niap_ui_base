...
export function ImageGenerationDialog({
  ...
  useEffect(() => {
    ...
    if (initialPrompt) {
      ...
      // Устанавливаем промт во все поля интерфейса
      setPrompt(cleanPrompt);
      setGeneratedPrompt(cleanPrompt); // Устанавливаем значение generatedPrompt
      
      // Если есть промт, переключаемся на вкладку произвольного запроса
      setActiveTab("prompt");
      console.log('Загружен промт из текущего поста:', cleanPrompt.substring(0, 100) + '...');
    } else {
      // Если промт не передан, обязательно сбрасываем оба поля
      setPrompt("");
      setGeneratedPrompt("");
      console.log('Сброс промта - новый пустой промт');
    }
    ...
    if (!initialPrompt) {
      ...
      console.log('Используем готовый промт из текущего поста');
    }
