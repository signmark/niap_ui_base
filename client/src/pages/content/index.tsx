    ...
    mutationFn: async ({ id, platforms }: { id: string, platforms?: {[key: string]: boolean} }) => {
      ...
      if (isStories) {
        ...
        if (selectedPlatform === 'instagram') {
          ...
          const hasMedia = hasImageUrl || hasVideoUrl || hasValidMedia;
          
          console.log('📄 Итоговый результат проверки медиафайлов для сторис:', { 
            hasImageUrl, 
            hasVideoUrl, 
            hasValidMedia, 
            итог: hasMedia ? 'МЕДИА НАЙДЕНО ✅' : 'МЕДИА ОТСУТСТВУЕТ ❌' 
          });
          
          if (!hasMedia) {
            // Формируем детальное сообщение об ошибке с рекомендациями
            const errorMessage = `Для публикации сторис в Instagram необходимо указать изображение или видео.
Проверка медиа: 
• Основное изображение: ${hasImageUrl ? 'найдено ✅' : 'отсутствует ❌'} 
• Основное видео: ${hasVideoUrl ? 'найдено ✅' : 'отсутствует ❌'} 
• Дополнительные медиа: ${hasValidMedia ? 'найдены ✅' : 'отсутствуют или невалидны ❌'}

Убедитесь, что добавлено хотя бы одно изображение или видео в поля "Изображение", "Видео" или "Дополнительные изображения".`;
        ...
        const current = useCampaignStore.getState().currentCampaign;
        console.log('🔍 Проверка ID кампании для публикации сторис:', {
          currentCampaignFromState: current?.id,
          alternativeCampaignId: currentContent?.campaignId
        });
        
        // Используем ID кампании из текущей кампании в сторе или из текущего контента
        const campaignIdToUse = current?.id || currentContent?.campaignId;
