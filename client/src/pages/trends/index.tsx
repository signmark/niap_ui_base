        ...
        checkIntervals.forEach((delay, index) => {
          setTimeout(() => {
            ...
            if (selectedTrendTopic?.id === trendId) {
              // Проверяем есть ли уже комментарии, если есть - не загружаем повторно
              if ((!trendComments || trendComments.length === 0) || index === 0) {
                loadTrendComments(trendId);
                console.log(`🔄 Проверка комментариев #${index + 1} для выбранного тренда ${trendId}`);
              } else {
                console.log(`✅ Комментарии уже загружены для тренда ${trendId}, пропускаем проверку #${index + 1}`);
              }
            } else {
  ...
  const { mutate: collectTrendsWithPlatforms, isPending: isCollecting } = useMutation({
    ...
    onSuccess: (data, { platforms, collectSources, collectComments }) => {
      ...
      if (collectComments && collectComments.length > 0) {
        console.log('🔄 Запущен сбор комментариев');
        
        if (selectedTrendTopic) {
          console.log('🔄 Выбран тренд:', selectedTrendTopic.id, '- проверяем комментарии');
                                  ...
                                  const analyzeSentiment = async () => {
                                    try {
                                      ...
                                      if (response.ok) {
                                        ...
                                        console.log('🔄 Обновляем кэш трендов и комментариев после сохранения анализа настроения');
                                        queryClient.invalidateQueries({ queryKey: ["trends"] });
