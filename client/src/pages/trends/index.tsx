  ...
  const collectTrendComments = async (trendId: string, trendUrl: string) => {
    try {
      ...
      if (response.ok) {
        ...
        // Автоматически проверяем комментарии через разумные интервалы
        console.log('🔄 Запущен сбор комментариев для тренда:', trendId);
        
        // Если этот тренд сейчас выбран - загружаем комментарии сразу и через 30 секунд
        if (selectedTrendTopic?.id === trendId) {
          console.log('🔄 Загружаем комментарии для выбранного тренда:', trendId);
          loadTrendComments(trendId);
          
          // Одна дополнительная проверка через 30 секунд (время обычно достаточное для N8N)
          setTimeout(() => {
            if (selectedTrendTopic?.id === trendId) {
              loadTrendComments(trendId);
              console.log(`🔄 Повторная проверка комментариев для тренда ${trendId}`);
            }
          }, 30000);
        } else {
          console.log('💡 Выберите этот тренд, чтобы увидеть собранные комментарии');
        }
      } else {
