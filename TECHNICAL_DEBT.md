...
## 0. Потеря данных о платформах при публикации на одну из платформ

### Обнаруженная проблема

При публикации контента на одну из запланированных платформ (например, Telegram), система теряет информацию о других запланированных платформах (например, VK и Instagram). Это происходит потому, что система не сохраняет существующие данные о других платформах при обновлении статуса для одной платформы.

### Причина появления

1. При получении контента из базы данных, система не преобразует поле `social_platforms` (snake_case) из базы данных в `socialPlatforms` (camelCase) для использования в приложении
2. При обновлении статуса публикации, система перезаписывает все поле `social_platforms` с данными только об одной платформе

### Рекомендация по устранению

1. Модифицировать метод `getCampaignContent` в хранилище для корректного преобразования `social_platforms` в `socialPlatforms`
2. Обновить метод `updatePublicationStatus` для сохранения всех существующих данных платформ при обновлении статуса для одной платформы
3. Создать единый процесс преобразования snake_case/camelCase для всех полей в системе

### Приоритет

Высокий - проблема приводит к потере данных и некорректной работе системы публикаций.

### Оценка трудозатрат

1-2 дня работы, включая исправление, тестирование и проверку на производстве.
