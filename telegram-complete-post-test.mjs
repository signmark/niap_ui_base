 ...
 * 
 * Запуск: node telegram-complete-post-test.mjs
 */
...
// Для загрузки модуля с процессором HTML
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Подключаем процессор HTML-кода для Telegram
import('./shared/telegram-html-processor.js').then(module => {
  const { processHtmlForTelegram } = module;
  ...
  async function sendImageWithHtmlCaption(imageUrl, caption) {
    ...
    try {
      ...
      const fileName = isLocalFile ? path.basename(imageUrl) : 'image.jpg';
      formData.append('photo', imageData, { filename: fileName });
...
Полезными перекусами являются те, которые обеспечивают чувство сытости, стабильный энергетический баланс и способствуют поддержанию здоровья. 🍏 Отличным вариантом перекуса являются орехи, йогурт, творог, хлебцы с авокадо и фрукты. 🍊 Даже кусочек тёмного шоколада считается полезным перекусом при употреблении в умеренных количествах. 🍫

Выбирайте перекусы, содержащие белок, полезные жиры и клетчатку – они дольше обеспечивают чувство сытости и поддерживают стабильный уровень веществ. 🥑

Не употребляйте пищу автоматически – перекус необходим, когда вы действительно испытываете легкое чувство голода, а не просто по привычке или от скуки. 🍎

При соблюдении этих рекомендаций перекусы станут полезными для здоровья, помогут контролировать аппетит и выстроить комфортную систему питания, предотвращающую как голод, так и переедание. 🌿`;

    // URL изображения для публикации (используем существующее изображение из репозитория)
    const imageUrl = './attached_assets/image_1741795103464.png';
  ...
  console.error(`Ошибка при импорте модуля: ${error.message}`);
});