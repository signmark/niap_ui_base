...
class TelegramPublisher {
  ...
  async sendDirectusImageToTelegram(imageUrl, chatId, caption, token) {
    try {
      ...
      // Проверка и подготовка caption
      if (!caption || caption.trim() === '') {
        this.log(`⚠️ Обнаружена пустая подпись, установка значения по умолчанию`);
        caption = "📷 Новая публикация";
      }
      
      this.log(`📋 Длина подписи: ${caption.length} символов`);
      ...
      try {
        ...
        // Шаг 2: Отправляем изображение с ID чата как есть и проверенной подписью
        try {
